...
public class GedcomParse {
				...
				if(str[0].equals("1")) {
					if(str[1].equals("NAME")) {
						...
						if(birthArray[0].equals("2") && birthArray[1].equals("DATE")) {//CREATE DATE FROM STRING
							...
							String death =individualList.get(individualList.size()-1).getDeath();
							int age;
							if(!death.equals("NA")) {
								String deathyear = death.substring(0,4);
								int yearDeath = Integer.parseInt(deathyear);
								age = yearDeath - Integer.parseInt(year);
							}else {
								age = yearNow - Integer.parseInt(year);
							}
						...
						if(deathArray[0].equals("2") && deathArray[1].equals("DATE")) {
							...
							String year = deathArray[4];
								...
								//set the age
								String birth = individualList.get(individualList.size()-1).getBirthday();
	...
	/*** *****************************************************************************************************
	 *  main method part done!
	 *  1.Constuctor/ 2.readFile()/ 3. writeIntoIndividualList() / 4.writeIntofamilyList() / 5.display()
	 *  
	 *  Please not change any code above!
	 *  If you need do some change for your Sprint test, please let the team know first!
	 *** *****************************************************************************************************/
		...
		for(Family one: familyList) {
			...
			for(String childId: childList) {
				if(!childId.equals("NA")) {
					...
					if(!checkOneDateAfterOneDateValid(birthStr, marrStr)) {
						//print the error message
					...
					if(!divoStr.equals("NA")) {
						if(!checkOneDateAfterOneDateValid(divoStr, birthStr)) {
							//print the error message
	...
	//--------Wenxuan part sprint 3 end----------------
	
	//--------Wenxuan part sprint 4----------------
			...
			for(String childID: childList) {
				if(!childID.equals("NA")) {
					...
					if(fatherAge-childAge>80) {
						System.out.println("ERROR: FAMILY: US12: "+father.getbirthdayLine()+": "+one.getID()+
								" : Husband's ("+father.getId()+ ") birth "+father.getBirthday()+
								" is more then 80 years old at the child ("+child.getId()+") birth "
								+child.getBirthday());
					}
					if(momAge-childAge>60) {
						System.out.println("ERROR: FAMILY: US12: "+mom.getbirthdayLine()+": "+one.getID()+
								" : Wife's ("+mom.getId()+ ") birth "+father.getBirthday()+
								" is more then 60 years old at the child ("+child.getId()+") birth "
								+child.getBirthday());
					}
				}
			}
...
//			String momName = mom.getName();
			...
			for(String childID: childList) {
				if(!childID.equals("NA")) {
//					String childName = (individualMap.get(childID)).getName();
//					if(fatherName.contentEquals(childName)) {
					if(fatherID.equals(childID)){
						System.out.println("ERROR: FAMILY: US17: "+mom.getIDLine()+": "
								+one.getID()+": "+"Parent should not marry the child: Husband ("+fatherID+") "
								+"has the same ID as the child ("
								+childID+") ");
					}
					//if(momName.contentEquals(childName)) {
					if(momID.equals(childID)){
						System.out.println("ERROR: FAMILY: US17: "+father.getIDLine()+": "
								+one.getID()+": "+"Parent should not marry the child: Husband ("+momID+") "
								+"has the same ID as the child ("
								+childID+") ");
					}
				}
	...
	public static void main(String[] args) {
		...
		proj3.readFile("/Users/wenxuanwang/Desktop/testCase.ged");
		...
		//I didn't change/delete any previous code, just comment it organize the whole file;
		
		
		
		
//		GedcomParse proj3 = new GedcomParse();
//		proj3.readFile("/Users/wenxuanwang/Desktop/testForSprint1.ged");
